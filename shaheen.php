...
$sql_personagem = "SELECT personagem_id, personagem_nome, personagem_img FROM alansteindorff_personagem WHERE personagem_nome = 'Shaheen' LIMIT 1";
$result_personagem = mysqli_query($conn, $sql_personagem);
$personagem = mysqli_fetch_assoc($result_personagem);

// Se não encontrar o personagem, exibe mensagem e encerra
if (!$personagem) {
...
<head>
  ...
  <link rel="stylesheet" href="tekkenlist.css" /> <!-- CSS principal -->
...
<body>
  ...
  <main class="char-movelist">
    ...
    <div class="inner-table">
      <div class="move-table">
        <?php $num = 1;
        // Loop para exibir todos os golpes do personagem
        while ($golpe = mysqli_fetch_assoc($result_golpes)): ?>
          <div class="move-card" id="golpe<?= $golpe['golpe_id'] ?>">

            <div class="move-info">
              <div class="move-title">
                <span class="move-number"><?= $num++ ?></span> <!-- Número do golpe -->
                <span class="move-name"><?= htmlspecialchars($golpe['golpe_nome']) ?></span> <!-- Nome do golpe -->
                <?php
              ...
              <div class="move-hit-dmg">
                <div class="move-hitlvlstring">
                  <?php
                  // Exibe o nível do hit (high, mid, low, etc) e propriedades especiais
                  if (!empty($golpe['golpe_hitlvl'])) {
                    ...
                    foreach ($hitlvls as $hlvl) {
                      ...
                      if (preg_match('/^([^\[]+)(?:\[(.+)\])?$/', $hlvl, $matches)) {
                        ...
                        $hit_class = '';
                        // Define classe CSS conforme o tipo de hit
                        ...
                        if ($prop) {
                          // Exibe ícone de propriedade especial (ex: power crush, heat, etc)
            ...
            <div class="move-extra">
              <div class="mv-section">
                ...
                <table class="move-frames">
                  <?php if (isset($_SESSION['usuarioNiveisAcessoId']) && ($_SESSION['usuarioNiveisAcessoId'] == 1 || $_SESSION['usuarioNiveisAcessoId'] == 2)): ?>
                    <!-- Botões de editar/excluir golpe (apenas ADM/gerente) -->
                    <div class="btn-edit-move" style="display: flex; gap: 8px;">
                      <a href="editar_golpe.php?id=<?= $golpe['golpe_id'] ?>&voltar=shaheen.php" class="goto-btn edit-btn">
                  ...
                  <tbody>
                    ...
                    <tr class="move-hitf">
                      ...
                      <?php
                      // Define classe CSS para o frame de hit (positivo, negativo, neutro)
                ...
                <?php if (isset($_SESSION['usuarioId'])): ?>
                  <!-- Botão para reportar erro (apenas logado) -->
                  <button class="report-btn"
                    onclick="openReportModal(<?= $golpe['golpe_id'] ?>, '<?= htmlspecialchars(addslashes($golpe['golpe_nome'])) ?>')">Report
                    Error</button>
                <?php else: ?>
  ...
  <!-- Legenda dos ícones usados nos golpes -->
  ...
  <script>
    // Função JS para abrir o modal de reporte de erro
